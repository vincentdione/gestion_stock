...
public interface TenantAwareRepository<T, ID> extends JpaRepository<T, ID> {
    ...
    @Query("select e from #{#entityName} e where e.id = :id and e.idEntreprise = ?#{@tenantContext.getCurrentTenant()}")
    @Override
    Optional<T> findById(@Param("id") ID id);

    @Query("select count(e) > 0 from #{#entityName} e where e.id = :id and e.idEntreprise = ?#{@tenantContext.getCurrentTenant()}")
